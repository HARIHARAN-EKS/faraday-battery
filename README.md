...
1. **Portable ZIP (recommended):** `dist\Faraday-1.0.2-portable-win64.zip`
   — extract anywhere and run `faraday.exe`. No install step, no admin
   rights, settings and history travel with the folder (USB-friendly);
   delete the folder to uninstall. VirusTotal: 0 detections, whole archive
   and every file inside. See [docs/PORTABLE.md](docs/PORTABLE.md).
2. **Installer (alternative):** `dist\Faraday-1.0.2-setup-win64.exe` —
