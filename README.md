...
1. **Portable ZIP (recommended):** `dist\Faraday-1.0.4-portable-win64.zip`
   — extract the **whole folder**, then run `faraday.exe` from inside it.
   No install step, no admin rights; settings and history travel with the
   folder (USB-friendly), and deleting the folder uninstalls it.
   VirusTotal (1.0.2 round): 0 detections, whole archive and every file
   inside. See [docs/PORTABLE.md](docs/PORTABLE.md).
2. **Installer (alternative):** `dist\Faraday-1.0.4-setup-win64.exe` —
...
> **Keep the program folder together.** `faraday.exe` is a small launcher;
> the Qt runtime lives beside it. Copying `faraday.exe` out on its own will
> not work — Faraday now tells you so with a clear message instead of a
> confusing Windows error.
